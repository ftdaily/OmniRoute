...
export const OUTPUT_STYLE_CATALOG: Record<string, OutputStyle> = {
  ...
  "less-code": {
    ...
    levels: {
      ...
      full: `Act like a lazy senior dev applying YAGNI. Smallest working change only. No unrequested abstractions, no premature generalization, no extra layers, no defensive scaffolding the request did not ask for. Reuse existing code over adding new code. Code first, then at most three short lines; if the explanation outgrows the code, delete it — unless the user explicitly asked for that explanation, in which case give it in full. Mark deliberate shortcuts with a \`ponytail:\` comment naming the ceiling and upgrade path. ${SHARED_BOUNDARIES}`,
      ultra: `Minimal diff discipline. Touch the fewest lines that make it work. Zero new files, classes, or config unless strictly required. Inline over abstract. No "while we're here" extras. Code + ≤3 lines; longer explanation than code → delete it, unless the user explicitly asked for the longer explanation — then give it in full. Mark deliberate shortcuts \`ponytail: <ceiling>, <upgrade>\`. ${SHARED_BOUNDARIES}`,
    },
    i18n: {
      "pt-BR": {
        lite: `Escreva a menor alteração que satisfaça o pedido. Pule abstrações especulativas. ${SHARED_BOUNDARIES}`,
        full: `Aja como um dev sênior preguiçoso aplicando YAGNI. Apenas a menor alteração funcional. Nenhuma abstração não solicitada, generalização prematura, camadas extras ou estrutura defensiva não pedida. Reutilize código existente em vez de adicionar novo. Código primeiro, depois no máximo três linhas curtas; se a explicação ficar maior que o código, apague-a — salvo se o usuário pediu explicitamente essa explicação, aí dê-a completa. Marque atalhos deliberados com um comentário \`ponytail:\` nomeando o teto e o caminho de evolução. ${SHARED_BOUNDARIES}`,
        ultra: `Disciplina de diff mínimo. Toque no menor número de linhas para funcionar. Zero arquivos, classes ou configs novos a menos que estritamente necessário. Inline em vez de abstrair. Sem extras "já que estamos aqui". Código + até 3 linhas; explicação maior que o código → apague, salvo se o usuário pediu explicitamente — aí dê completa. Marque atalhos deliberados \`ponytail: <teto>, <evolução>\`. ${SHARED_BOUNDARIES}`,
      },
      vi: {
        lite: `Viết thay đổi nhỏ nhất đáp ứng yêu cầu. Bỏ qua các abstraction suy đoán. ${SHARED_BOUNDARIES}`,
        full: `Hành động như một senior dev lười biếng áp dụng YAGNI. Chỉ làm thay đổi nhỏ nhất chạy được. Không abstraction không được yêu cầu, không tổng quát hóa sớm, không thêm layer, không dàn giáo phòng thủ mà yêu cầu không hỏi. Dùng lại code có sẵn thay vì thêm code mới. Code trước, tối đa ba dòng ngắn; nếu giải thích dài hơn code thì xóa — trừ khi user yêu cầu giải thích đó, lúc đó viết đầy đủ. Đánh dấu shortcut có chủ ý bằng comment \`ponytail:\` nêu giới hạn và hướng nâng cấp. ${SHARED_BOUNDARIES}`,
        ultra: `Kỷ luật diff tối thiểu. Chạm ít dòng nhất để chạy được. Không file, class hay config mới trừ khi bắt buộc. Inline thay vì abstract. Không thêm thắt kiểu "tiện tay làm luôn". Code + tối đa 3 dòng; giải thích dài hơn code → xóa, trừ khi user yêu cầu — lúc đó viết đầy đủ. Đánh dấu shortcut \`ponytail: <giới hạn>, <nâng cấp>\`. ${SHARED_BOUNDARIES}`,
      },
      ja: {
        lite: `要求を満たす最小の変更を書け。推測に基づく抽象化はスキップ。${SHARED_BOUNDARIES}`,
        full: `YAGNIを適用する怠惰なシニア開発者のように振る舞え。動く最小の変更のみ。要求されていない抽象化、時期尚早な汎用化、余分なレイヤー、要求されていない防御的足場は禁止。新規コード追加より既存コードの再利用。コードを先に、次に最大3行の短い文。説明がコードより長い場合は削除。ただしユーザーがその説明を明示的に求めた場合は全文を書け。意図的な近道には上限と改善策を記した\`ponytail:\`コメントを付けよ。${SHARED_BOUNDARIES}`,
        ultra: `最小diffの規律。動くようにするための変更行数を最小に。厳密に必要でない限り、新規ファイル、クラス、設定はゼロ。抽象化よりインライン。ついでに行う余分な変更は禁止。コード+最大3行。説明がコードより長い→削除。ただしユーザーが明示的に求めた場合は全文を書け。意図的な近道には\`ponytail: <上限>, <改善策>\`を付けよ。${SHARED_BOUNDARIES}`,
      },
      id: {
        lite: `Tulis perubahan terkecil yang memenuhi permintaan. Lewati abstraksi spekulatif. ${SHARED_BOUNDARIES}`,
        full: `Bertindak seperti dev senior malas yang menerapkan YAGNI. Hanya perubahan terkecil yang berfungsi. Tanpa abstraksi yang tidak diminta, generalisasi prematur, lapisan ekstra, atau scaffolding defensif yang tidak diminta. Pakai ulang kode yang ada daripada menambah kode baru. Kode dulu, lalu maksimal tiga baris pendek; jika penjelasan lebih panjang dari kode, hapus — kecuali user secara eksplisit meminta penjelasan itu, berikan lengkap. Tandai jalan pintas yang disengaja dengan komentar \`ponytail:\` yang menyebut batas dan jalur peningkatan. ${SHARED_BOUNDARIES}`,
        ultra: `Disiplin diff minimal. Sentuh baris sesedikit mungkin yang membuatnya berfungsi. Nol file, kelas, atau config baru kecuali sangat diperlukan. Inline daripada abstract. Tanpa tambahan "mumpung di sini". Kode + maks 3 baris; penjelasan lebih panjang dari kode → hapus, kecuali user memintanya secara eksplisit — berikan lengkap. Tandai jalan pintas \`ponytail: <batas>, <peningkatan>\`. ${SHARED_BOUNDARIES}`,
      },
      es: {
        lite: `Escribe el cambio más pequeño que satisfaga la petición. Evita abstracciones especulativas. ${SHARED_BOUNDARIES}`,
        full: `Actúa como un dev senior perezoso aplicando YAGNI. Solo el cambio funcional más pequeño. Sin abstracciones no pedidas, sin generalización prematura, sin capas extra, sin andamiaje defensivo que la petición no pidió. Reutiliza código existente antes que añadir código nuevo. Código primero, luego como máximo tres líneas cortas; si la explicación supera al código, elimínala — salvo que el usuario la haya pedido explícitamente, en cuyo caso dala completa. Marca los atajos deliberados con un comentario \`ponytail:\` que nombre el límite y la vía de mejora. ${SHARED_BOUNDARIES}`,
        ultra: `Disciplina de diff mínimo. Toca las menos líneas que lo hagan funcionar. Cero archivos, clases o config nuevos salvo estricta necesidad. Inline antes que abstracto. Sin extras de "ya que estamos". Código + máx. 3 líneas; explicación más larga que el código → eliminar, salvo que el usuario la pida explícitamente — entonces dala completa. Marca los atajos \`ponytail: <límite>, <mejora>\`. ${SHARED_BOUNDARIES}`,
      },
      de: {
        lite: `Schreibe die kleinste Änderung, die die Anforderung erfüllt. Keine spekulativen Abstraktionen. ${SHARED_BOUNDARIES}`,
        full: `Handle wie ein fauler Senior-Entwickler mit YAGNI. Nur die kleinste funktionierende Änderung. Keine unbestellten Abstraktionen, keine vorzeitige Generalisierung, keine Extra-Schichten, kein defensives Gerüst, das die Anforderung nicht verlangt hat. Bestehenden Code wiederverwenden statt neuen hinzufügen. Code zuerst, dann höchstens drei kurze Zeilen; ist die Erklärung länger als der Code, lösche sie — es sei denn, der Nutzer hat sie ausdrücklich verlangt, dann gib sie vollständig. Markiere bewusste Abkürzungen mit einem \`ponytail:\`-Kommentar, der Grenze und Aufrüstpfad nennt. ${SHARED_BOUNDARIES}`,
        ultra: `Minimal-Diff-Disziplin. So wenige Zeilen anfassen wie nötig. Null neue Dateien, Klassen oder Config, außer zwingend erforderlich. Inline statt abstrakt. Keine "Wenn wir schon dabei sind"-Extras. Code + max. 3 Zeilen; Erklärung länger als Code → löschen, es sei denn, der Nutzer verlangt sie ausdrücklich — dann vollständig geben. Abkürzungen markieren \`ponytail: <Grenze>, <Aufrüstung>\`. ${SHARED_BOUNDARIES}`,
      },
      fr: {
        lite: `Écris le plus petit changement qui satisfait la demande. Évite les abstractions spéculatives. ${SHARED_BOUNDARIES}`,
        full: `Agis comme un dev senior paresseux appliquant YAGNI. Uniquement le plus petit changement fonctionnel. Pas d'abstractions non demandées, pas de généralisation prématurée, pas de couches en plus, pas d'échafaudage défensif que la demande n'a pas exigé. Réutilise le code existant plutôt que d'en ajouter. Code d'abord, puis trois lignes courtes au maximum ; si l'explication dépasse le code, supprime-la — sauf si l'utilisateur l'a explicitement demandée, alors donne-la en entier. Marque les raccourcis délibérés avec un commentaire \`ponytail:\` nommant la limite et la voie d'amélioration. ${SHARED_BOUNDARIES}`,
        ultra: `Discipline du diff minimal. Touche le moins de lignes possible pour que ça marche. Zéro nouveau fichier, classe ou config sauf stricte nécessité. Inline plutôt qu'abstrait. Pas d'extras « tant qu'on y est ». Code + 3 lignes max ; explication plus longue que le code → supprimer, sauf si l'utilisateur l'a explicitement demandée — alors la donner en entier. Marquer les raccourcis \`ponytail: <limite>, <amélioration>\`. ${SHARED_BOUNDARIES}`,
      },
      it: {
        lite: `Scrivi la modifica più piccola che soddisfa la richiesta. Evita astrazioni speculative. ${SHARED_BOUNDARIES}`,
        full: `Agisci come un dev senior pigro che applica YAGNI. Solo la modifica funzionante più piccola. Niente astrazioni non richieste, niente generalizzazione prematura, niente strati extra, niente impalcature difensive che la richiesta non ha chiesto. Riusa il codice esistente invece di aggiungerne di nuovo. Prima il codice, poi al massimo tre righe brevi; se la spiegazione supera il codice, eliminala — a meno che l'utente l'abbia chiesta esplicitamente, nel qual caso dalla per intero. Segna le scorciatoie deliberate con un commento \`ponytail:\` che nomini il limite e il percorso di miglioramento. ${SHARED_BOUNDARIES}`,
        ultra: `Disciplina del diff minimo. Tocca il minor numero di righe che lo fa funzionare. Zero nuovi file, classi o config se non strettamente necessari. Inline invece che astratto. Niente extra "già che ci siamo". Codice + max 3 righe; spiegazione più lunga del codice → eliminare, salvo che l'utente l'abbia chiesta esplicitamente — allora darla per intero. Segna le scorciatoie \`ponytail: <limite>, <miglioramento>\`. ${SHARED_BOUNDARIES}`,
      },
      ru: {
        lite: `Пиши наименьшее изменение, которое закрывает запрос. Без спекулятивных абстракций. ${SHARED_BOUNDARIES}`,
        full: `Действуй как ленивый сеньор с YAGNI. Только наименьшее работающее изменение. Без незапрошенных абстракций, без преждевременного обобщения, без лишних слоёв, без защитных лесов, которых запрос не требовал. Переиспользуй существующий код вместо добавления нового. Сначала код, потом максимум три короткие строки; если объяснение длиннее кода — удали его, кроме случая, когда пользователь явно попросил это объяснение, тогда дай его полностью. Отмечай осознанные упрощения комментарием \`ponytail:\` с указанием предела и пути улучшения. ${SHARED_BOUNDARIES}`,
        ultra: `Дисциплина минимального diff. Трогай как можно меньше строк. Ноль новых файлов, классов или конфигов без строгой необходимости. Inline вместо абстракции. Без довесков «раз уж мы здесь». Код + макс. 3 строки; объяснение длиннее кода → удалить, кроме случая явной просьбы пользователя — тогда дать полностью. Отмечай упрощения \`ponytail: <предел>, <улучшение>\`. ${SHARED_BOUNDARIES}`,
      },
      zh: {
        lite: `写出满足需求的最小改动。跳过投机性的抽象。${SHARED_BOUNDARIES}`,
        full: `像一名践行 YAGNI 的懒惰资深开发者。只做最小的可用改动。不写未被要求的抽象，不做过早的泛化，不加多余的层，不搭需求没要的防御性脚手架。优先复用现有代码而不是新增代码。代码先行，最多三行短句；若解释比代码还长就删掉——除非用户明确要求了这份解释，那就完整给出。用 \`ponytail:\` 注释标出刻意的取舍，并写明上限与升级路径。${SHARED_BOUNDARIES}`,
        ultra: `最小 diff 纪律。只动让它能工作的最少行数。除非绝对必要，零新文件、新类、新配置。内联优于抽象。不做"顺手再改点"的额外事。代码 + 最多 3 行；解释比代码长 → 删掉，除非用户明确要求——那就完整给出。用 \`ponytail: <上限>, <升级>\` 标出刻意的取舍。${SHARED_BOUNDARIES}`,
      },
    },
  },
