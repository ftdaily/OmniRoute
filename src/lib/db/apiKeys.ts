 ...
 */
export async function isComboNameAllowedForKey(
  key: string | null | undefined,
  ...
  const { modelAccessMode, allowedModels, blockedModels } = metadata;
  const candidates = [comboName];

  // Deny-list wins over any allow-list entry.
  if (blockedModels?.some((pattern) => modelPatternMatches(pattern, candidates))) {
    return false;
  }

  // Empty allow-list only permits access when the key is not restricted.
  if (!allowedModels || allowedModels.length === 0) {
    return modelAccessMode !== "restricted";
  }

  return allowedModels.some((pattern) => modelPatternMatches(pattern, candidates));
}
