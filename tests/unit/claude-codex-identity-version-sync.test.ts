...
test("Codex client is pinned to the captured 0.153.2 release", () => {
  assert.equal(codexCfg.getCodexClientVersion(), "0.153.2");
  assert.equal(codexCfg.getCodexUserAgent(), "codex-cli/0.153.2 (Windows 10.0.26200; x64)");
  assert.equal(codexCfg.getCodexDefaultHeaders().Version, "0.153.2");
  assert.equal(codexCfg.getCodexCliRsHeaders()["User-Agent"], "codex_cli_rs/0.153.2");
});
